...
#if __STDC__
   void ztpmid_(char *oper, int *blkno, int *fcb, char *buff,
      int *nbytes, int *ierr)
#else
   void ztpmid_(oper, blkno, fcb, buff, nbytes, ierr)
   char oper[], buff[];
   int *fcb, *blkno, *nbytes, *ierr;
#endif
/*--------------------------------------------------------------------*/
/*! read/write 2880/2560 byte blocks of data from/to disk             */
/*# Z Tape                                                            */
/*--------------------------------------------------------------------*/
/*;  Copyright (C) 1995, 1997-1999, 2001, 2009                        */
...
/*--------------------------------------------------------------------*/
/*  Read/write "nbytes" of data from/to disk at FITS block blkno      */
/*                                                                    */
...
/*     blkno    I      Block number to read/write                     */
...
/*--------------------------------------------------------------------*/
   ...
   *(fcb + Z_fcbxfr) = 0;
   ...
   if (*(fcb + Z_fcbreq) != 0) {
                                        ...
                                        /* is 2880.                   */
     if ((*(fcb + Z_fcbreq) != 2880) && (*(fcb + Z_fcbreq) != 2560)) {
         *ierr = 2;
         }
      else {
         ...
         firstbyte = (*blkno - 1);
         firstbyte = firstbyte * (*(fcb + Z_fcbreq));
