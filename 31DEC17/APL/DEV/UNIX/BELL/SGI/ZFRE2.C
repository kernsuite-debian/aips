...
/*                                                                    */
/*  SGI version:  Cloned from generic version.  No longer dependent   */
/*  -----------   on output from "df".  Use "statfs()" instead.       */
/*  This version supports filesystems >2Gbytes                        */
/*  SG-Irix has a different calling sequence for statfs().  Grr.      */
/*                                                                    */
  ...
  if ((buf = (struct statfs *) calloc (1, sizeof(struct statfs))) == 0)
   ...
   for (i = 0; i < *nvol && i < 35; i++) {
	 ...
	 sprintf(vlist[i], "undefined %s", cmd);
         ...
         if ((istat = statfs(vtemptr, buf, sizeof(*buf), 0)) == 0) {
	    ...
	    xm = buf->f_bsize / (512. * Z_nbpab) / 1024.0;
            ...
            frblks[i] = xm * buf->f_bfree + 0.5;
            tot_user_blocks = buf->f_blocks;
            if (tot_user_blocks > 0)
               pctful[i] = 100.5 - (buf->f_bfree * 100.) / tot_user_blocks;
            else
               pctful[i] = 0.;
         } else {
