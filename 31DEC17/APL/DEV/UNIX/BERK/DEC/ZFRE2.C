...
/*# IO-basic                                                          */
/*--------------------------------------------------------------------*/
/*;  Copyright (C) 1995, 1997-1998, 2009                              */
...
/*                                                                    */
/*  DEC version:  No longer dependent on output from "df".  Use       */
/*  -----------  "statfs()" instead.  This is the DEC version, for    */
/*  OSF/1 where the statfs() call needs an extra argument :-(         */
/*  otherwise it's pretty much identical to the generic unix version. */
/*  Supports >2G partitions.                                          */
/*  Added support for advfs (was giving 512-byte blocks!)             */
/*--------------------------------------------------------------------*/
{
                                        /* # bytes per AIPS-byte      */
   extern int Z_nbpab;
...
/*--------------------------------------------------------------------*/
   ...
   for (i = 0; i < *nvol && i < 35; i++) {
	 ...
	 sprintf(vlist[i], "undefined %s", cmd);
         ...
         if ((istat = statfs(vtemptr, &buf,
			     sizeof(struct statfs))) == 0) {
                                        /* Unix blocks to bytes,      */
                                        /* thence to aips blocks.     */
                                        /* Note here buf.fd_req.xxx,  */
                                        /* on other UNIXen buf.f_xxx  */
                                        /* in most cases.  DEC used to*/
                                        /* have btot in 1k blocks (see*/
                                        /* man page for getmnt(2)) but*/
                                        /* now use f_bfize.           */
                                        /* Avoid overflow via "xm"    */
	    xm = buf.f_fsize / (512. * Z_nbpab) / 1024.0;
