zrmtap_()
/*--------------------------------------------------------------------*/
/*! support for remote tape drives                                    */
/*# IO-basic Tape                                                     */
/*--------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------*/
/*  This set of I/O calls is public domain software packaged for use  */
/*  in AIPS.  For references, see below.                              */
/*                                                                    */
/*  The remote tape capability is selected by defining the option     */
/*  REMOTE_TAPES in CCOPTS.SH.                                        */
/*                                                                    */
/*  I/O calls for tape drive functions are intercepted by the         */
/*  routines in this module.  The device name for the open is of the  */
/*  form                                                              */
/*     [<system>:]/dev/<device>                                       */
/*  If the system name is absent, then all subsequent commands to the */
/*  device are performed on the local system.  If the system name is  */
/*  given, then a pipe is opened to that system, and all subsequent   */
/*  operations on that device are passed to the remote system.  The   */
/*  account running AIPS must have read/write access to the device on */
/*  the remote system.  If this is not the case, consult your system  */
/*  manager.  The more general form of the remote device name is      */
/*  given in comments in _rmt_open below.                             */
/*                                                                    */
/*  Restrictions.                                                     */
/*  1. In this scheme, the actual I/O commands that are used locally  */
/*  are transmitted to the remote system.  There is no translation.   */
/*  Thus this protocol will only function between systems whose basic */
/*  tape operations are identical.  AIPS only uses a few basic tape   */
/*  commands.  These have been shown to be the same on several        */
/*  systems, but it is not guaranteed.  In order to be certain that   */
/*  this will work between systems of two different vendors, the      */
/*  contents of /usr/include/sys/mtio.h (or equivalent) should be     */
/*  identical for those basic commands on both systems.               */
/*  2. Multiple buffering is not supported in this scheme.  If remote */
/*  tape support is selected, then AIPS must turn off multiple        */
/*  buffering for all tape operations, both local and remote.         */
/*                                                                    */
