...
\renewcommand{\titlea}{31-December-2017 (revised 18-December-2017)}
...
at a time, so specity {\tt nchan $N$ \CR} to load the data with $N$
...
flags ({\tt DOPLOT} $\leq 0$).  {\tt RFLAG} has numerous {\tt FPARM}
options you may choose.  {\tt FPARM(9)} and {\tt FPARM(10)} scale the
rms for {\tt NOISE} and {\tt SCUTOFF} that {\tt RFLAG} computes.  If
the fraction of a channel that is flagged exceed {\tt FPARM(7)}, the
entire channel is flagged. {\tt FPARM(6)} is especially useful at
P-band as it allows channels adjacent to an RFI peak to be flagged.
Similarly, {\tt FPARM(8)} allows channels between strong RFI peaks to
be flagged.  Read the help file for detailed information.  Flagging
calibrator scans aggressively is recommended especially for
...
\dispe{If these plots are too crowded, you can interactively select
  ...
  if desired.  If you do, then you should re-run {\tt CALIB}\@.o}
...
\dispe{To use {\tt EDITA} to look at both amplitude and phase at the
  same time for multiple antennas}
\dispi{default edita \CR}{ }
\dispi{indi $d$; getn 6 \CR}{ }
\dispi{inext 'sn' ; dotwo 1 \CR}{to plot the phases in the new table.}
\dispi{crowded 1 ; do3col 1 \CR}{to combine all IFs in each colorful
...
\dispe{One also needs a ``meaningfull'' single-plane image, perhaps
...
\dispe{This allows you to look at spectra from the cube at individual
  ...
  frequencies.  Enter {\tt EXPLAIN TVSPC} or see \AIPS Memo 120 for
  details.}
