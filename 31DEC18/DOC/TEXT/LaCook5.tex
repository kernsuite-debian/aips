%-----------------------------------------------------------------------
%;  Copyright (C) 1995, 1997-1998, 2000-2016, 2018
...
an environment variable, \eg\ {\tt MYAREA}, to point at the disk
...
commands described in \Sec{tapeuse} to position the tape to the
...
\dispx{ }{to specify the pixel coordinates of the Clean windows
          as leftmost $x$, bottom-most $y$, rightmost $x$, topmost $y$
...
\dispt{\tndx{OBOXFILE}\qs '$area:outfilename$' \CR}{to specify the name
          ...
          environment variable followed by a colon to be assigned a
...
or rectangular box which defines a region in which Clean components are
...
\dispx{IMSTAT}{to compute and display robust and normal statistics on
         the current residual image}
...
\dispx{CONTINUE CLEAN}{to resume Cleaning now rather than wait for the
          time out period.}
\dispe{A blank {\tt OBOXFILE} adverb causes a temporary one to be
...
file in each iteration and the new {\tt SN} table contains all of the
...
successful program with EVLA wide-band data (\Sec{rflag}).  Task
...
which is easy for Linux and not too hard for Mac systems, then you may
use these programs.  Two of the verbs are used to translate EVLA data
into \AIPS\ format; see \Sec{BDF2aips}.  The other four verbs provide
