%-----------------------------------------------------------------------
%;  Copyright (C) 2000-2008, 2013-2015, 2018
...
\renewcommand{\titlea}{31-December-2018 (revised 19-July-2018)}
...
\begin{itemize}
\item {{\tt \tndx{VLBALOAD}}: loads VLBA data with simplified inputs}
\item {{\tt \tndx{VLBASUMM}}: makes summary listings of your data set}
\item {{\tt \tndx{VLBAFIX}}: Fixes VLBA data}
\item {{\tt \tndx{VLBATECR}}: automatically downloads and applies ionospheric
           corrections}
\item {{\tt \tndx{VLBAEOPS}}: automatically downloads and applies corrections
           to the Earth Orientation Parameters used by the
           correlator}
\item {{\tt \tndx{VLBACCOR}}: determines amplitude corrections caused by
           errors in sampler thresholds}
\item {{\tt \tndx{VLBAPANG}}: determines phase corrections for parallactic
           angles}
\item {{\tt \tndx{VLBAPCOR}}: determines instrumental phase corrections using
           pulse cals}
\item {{\tt \tndx{VLBAMPCL}}: determines instrumental phase corrections using
           {\tt FRING}}
\item {{\tt \tndx{VLBACPOL}}: calibrates cross polarization delays}
\item {{\tt \tndx{VLBABPSS}}: does bandpass calibration}
\item {{\tt \tndx{VLBAAMP}}: does adjustment due to normalization errors and
            a-priori amplitude calibration}
\item {{\tt \tndx{VLBAFRNG}}: does global fringe fit using {\tt FRING}}
\item {{\tt \tndx{VLBAFRGP}}: does global fringe fit for phase referenced
           experiments using {\tt FRING}}
\item {{\tt \tndx{VLBASNPL}}: plots the {\tt SN} or {\tt CL} tables versus
           time}
\item {{\tt \tndx{VLBACRPL}}: plots the cross-correlation spectrum}
\end{itemize}
...
this correction.  {\tt \tndx{VLBAEOPS}} automatically downloads a file
...
{\tt \tndx{VLBATECR}}\@.  This procedure automatically downloads
...
spectral line data described in \Sec{acfit} and \Sec{lineFRING}, but
...
gain curve information can usually be obtained in {\tt \tndx{ANTAB}}
format and loaded with the \AIPS\ task {\tt ANTAB}.  \Sec{nonvlba} has
information on including non-VLBA calibration information, if they are
not already included in the data.  \Sec{vlba+vla} has details on how
to incorporate the pre-EVLA \Indx{VLA} $T_{\rm sys}$ and gain curves.
Otherwise consult \Sec{antabformat}\@.
...
and {\tt FRING} without using {\tt
  VLBAPCOR}\@.\Todx{VLBAUTIL}\Todx{VLBAPCOR}\Iodx{VLBI}}
...
completely deleted from the data set.\Todx{VLBAMPCL}}

\item\ {Now you {\it must} check the results of correcting your
instrumental delays using {\tt \Tndx{VLBACRPL}} or {\tt POSSM}\@.  Set
...
{\tt \Tndx{VLBAAMP}}\@. This procedure runs several tasks: {\tt
...
\Sec{vlbcamp} for a more detailed discussion of {\tt APCAL}\@. Lastly,
...
these messages.\Iodx{VLBA}\iodx{VLBI}\Todx{VLBAUTIL}}
...
\Rchap{cal} and \Rappen{Olderdata}).  Determine the flux density of a
...
\Item {Toss 6 {\bf chicken breasts} cooked and cubed with banana mayo;
     divide onto salads.}
\Item {Sprinkle with 2 bunches ($\approx 48$) halved {\bf grapes} and
