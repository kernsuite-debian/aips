...
\renewcommand{\titlea}{31-December-2018 (revised 18-July-2018)}
...
to simplify some of the VLBI reduction steps are mentioned and become
available to you after you enter the command {\us RUN VLBAUTIL \CR}\@.
There is also a VLBA pipeline, {\tt VLBARUN}, which is useful for
simple datasets, see \Sec{vlbarun}.  If you have very old VLBI data,
in Mk III or Mk IV format, see \Sec{oldvlb}.  We also include here
some background information concerning the structure of VLBI data
sets, the data  reduction philosophy and a description of some of the
effects for which corrections must be determined and applied.  It is
important to understand these aspects if you wish to reduce your data
reliably.  For more background information on VLBI data reduction
consult {\it VLBI and the VLBA\/}, Astronomical Society of the Pacific
(ASP) Conference Series No. 82, 1995.
...
see \Sec{oldvlb}.  Data from the Penticton correlator should be loaded
using {\tt FITLD}, sorted ({\tt MSORT}, \Sec{vlbsort}), and indexed
({\tt INDXR}, \Sec{vlbINDXR}).
...
\dispt{OUTFILE\qs '{it /directoryname}}{to specify directory for
             ...
             the beginning.}
\dispt{OUTTEXT\qs '{it email@somewhere.edu}}{to specify an e-mail
...
require multiple passes through the correlator will have one file per
observing mode {\it per correlation pass\/}.  Data from separate
correlator passes can be concatenated using task {\tt VBGLU} and/or
merged with task {\tt VBMRG}\@.\Iodx{VLBA}}\Iodx{VLBI}
...
which is part of {\tt VLBAUTIL}, called {\tt \Tndx{VLBATECR}} that
...
The dispersive delays should be checked using {\tt \tndx{EDITA}}
(option {\tt DODELAY=3}), {\tt SNPLT} (options {\tt INEXT 'CL'; INVERS
2; OPTY 'DDLY'}) and {\tt VPLOT} (options {\tt BPARM 0; APARM 0; DOCAL
1; GAINUSE 2}).  {\tt \Tndx{TECOR}} is only as good as the models,
which at this time are quite rough.  Therefore, it is a very good idea
to compare the corrected and uncorrected phases using {\tt
VPLOT}\@.\Iodx{VLBI}
...
\dispt{INFILE\qs 'FITS:usno\_finals.erp \CR}{to specify file with
               ...
               lower case letters.}
\dispt{GO \CR}{to run the program.}
\dispe{In case the automatic download fails, {\tt VLBAEOPS} can use
the file which you have downloaded manually.}
...
\dispt{TIMERANG\qs 0 \CR}{to plot all times.}
...
\Sec{tyapl}), the known gain curve ({\tt GC} table), and the first
...
edit or smooth the {\tt SY} table see \Sec{tyapl}.  Then run {\tt
...
The VLBA system can inject narrow band signals (``phase-cals'') into
the data recorded at each antenna from which the IF channel phase
offsets, and the instrumental single-band delays, can be
determined.\iodx{fringe-fitting}
...
\dispt{INDISK\qs{\it n\/} ; GETN\qs {\it ctn\/} \CR}{to specify the
           input file.}
\dispt{TIMERANG\qs {\it d1 h1 m1 s1 d2 h2 m2 s2\/} \CR}{to specify a
...
\dispt{GAINUSE\qs {\it CLin\/} \CR}{to indicate the {\tt CL} table
           with all calibration up to this point.}
\dispt{OPCODE\qs 'CALP' \CR}{to indicate that there are antennas
           with no fringes for the scan in {\tt TIMERANG}; use
...
{\tt BPVER} and {\tt DOBAND} as described in \Sec{BPASSit} when running
...
\dispt{INDISK\qs{\it n\/} ; GETN\qs {\it ctn\/} \CR}{to specify the
           input file.}
\dispt{TIMERANG\qs 0}{to include all times.}
...
\dispt{DPARM(2)\qs 10000 \CR}{to select a delay window in nsec,
            ...
            range.  For a 250 kHz-bandwidth observation, setting this
...
convenient to use tasks {\tt \tndx{SCMAP}} or {\tt \tndx{SCIMG}} which
cycle between imaging and self-calibration with an option to edit the
$UV$ data.  If you wish to edit the Clean components or make plots,
you will need to do the manual Clean/self-cal process with {\tt IMAGR}
and {\tt CALIB}\@.}
...
control {\tt \tndx{CALIB}} and {\tt \tndx{IMAGR}}\@.  Again, please
note that the latter is preferable to all previous imaging tasks; {\tt
\tndx{SCIMG}} and to a lesser extent {\tt \tndx{SCMAP}} offer the same
