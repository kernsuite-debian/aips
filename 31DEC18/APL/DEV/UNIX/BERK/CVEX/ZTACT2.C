...
/*	 printf("\nDEBUG: dead lockfile, removing %s\n", pathname); */
	 fflush(stdout);
         unlink(pathname);
         }
      }
...
*/
               ...
               cpid[i] = '\0';
/*             sscanf(cpid,"%d",&procid); no, pid_t is a short on convex */
               sscanf(cpid,"%hd",&procid);
                                        /* Process active             */
                                        /* Return process id          */
               if ((procid > 1) &&
...
/*	 	  printf("\nDEBUG: dead lockfile %s, (filename %s, cpid %s) errno %d, procid %d\n", */
/*			 pathname, filename, cpid, errno, procid); */
/*		  fflush(stdout); */
                  unlink(pathname);
                  }
               }
