...
/*                        4 => other                                  */
/*  SGI version, adapted from 4.2 bsd UNIX version (uses "statfs").   */
/*  Does not use statfs.f_bavail, use f_bfree.                        */
/*--------------------------------------------------------------------*/
char pname[MAXPNAME];
         ...
         if (stat (xlated, &statbuf) == -1) {
            ...
            *ierr = 4;
            perror ("ZCREA2 (stat)");
                                        ...
                                        /* target file system.        */
                                        /* SG-IRIX version, more args */
         else if (statfs (xlated, &statfsbuf,
            ...
            *ierr = 4;
            ...
            blocks = 1 + (bytes-1) / statfsbuf.f_bsize;
                                        /* Enough room?  f_bavail may */
                                        /* not be available on SGI... */
                                        /* if so, use f_bfree         */
            if (statfsbuf.f_bfree < blocks) {
                                        /* Insufficient space.        */
               *ierr = 3;
            }
            else {
