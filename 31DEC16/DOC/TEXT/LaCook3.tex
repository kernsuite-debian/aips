%-----------------------------------------------------------------------
%;  Copyright (C) 1995-1998, 2000-2009, 2012-2016
...
\renewcommand{\titlea}{31-December-2015 (revised 11-November-2016)}
...
adverb.  In {\tt 31DEC12}, {\tt \tndx{M2CAT}} and {\tt \tndx{U2CAT}}
list files based on the {\tt IN2DISK} adverb.  {\tt \tndx{M3CAT}},
{\tt \tndx{U3CAT}}, {\tt \tndx{M4CAT}}, {\tt \tndx{U4CAT}}, {\tt
\tndx{OCAT}}, and {\tt \tndx{UOCAT}} are similar for the {\tt
IN3DISK}, {\tt IN4DISK}, and {\tt OUTDISK} adverbs, respectively.}
...
\dispt{INDISK \qs {\it n} ; GETN \qs {\it ctn} \CR}{to select the file
                  to be displayed.}
\dispt{PRTASK \qs 'UVMAP' \CR}{to examine only history information
                  from {\tt UVMAP}.}
\dispt{DOCRT \qs 1 \CR}{to direct the display to your terminal, using
                  its full width.}
\dispt{\tndx{PRTHI} \CR}{to print the {\tt UVMAP} history.}
...
\dispt{DOALL \qs $m$ ; GO\qs DISKU \CR}{to run the job, listing all
              catalog entried requiring more than $m$ Megabytes.}
\etd
...
{\tt UVMAP}, {\tt APCLN}, and {\tt VTESS}, the data editor {\tt TVFLG}
and the sorter {\tt UVSRT} may create large scratch and ``work''
...
Note too that, if you are using more than one computer on a given disk
area, only those scratch files created by your current computer will
be deleted when you run the {\tt SCRD} verb.  Work files have to be
deleted individually since they can be still of use after the task
which created them has finished.}
...
\dispt{\tndx{HELP VERBS}\CR}{to get a list of all verbs, pseudoverbs,
...
frequenctly used; see \Sec{externfile}.  Tape usage in \AIPS\ is
