...
#if __STDC__
   void zfi2_(char *oper, int *fcb, char *buff, int *nrec, char *msgbuf,
      int *ierr)
#else
   void zfi2_(oper, fcb, buff, nrec, msgbuf, ierr)
   char oper[], buff[], msgbuf[];
   int *fcb, *nrec, *ierr;
#endif
/*--------------------------------------------------------------------*/
...
/*  Output:                                                           */
/*     msgbuf C*80   extra error message (null terminated)            */
/*     ierr   I      Error return code: 0 => no error                 */
/*                      2 => bad opcode                               */
/*                      3 => I/O error                                */
/*                      4 => end of file                              */
/*                   1000 + n => read only n (< 256) words)           */
/*  Generic UNIX version now call msgwrt so MSGWRT cannot use         */
/*--------------------------------------------------------------------*/
{
   ...
   if (checkbytes >= sizebytes) {
      ...
      *ierr = 3;
      if (sizeof(long) < sizeof(off_t)) {
         sprintf (msgbuf,"ZFI2: REQUEST FOR BYTES %lld THRU = %lld",
            firstbyte + 1, lastbyte + 1);
         zmsgwr_ (msgbuf, &n7);
         sprintf (msgbuf,"ZFI2: BEYOND EOF = %lld", sizebytes);
         }
      else if (sizeof(int) < sizeof(long)) {
         sprintf (msgbuf,"ZFI2: REQUEST FOR BYTES %ld THRU = %ld",
            firstbyte + 1, lastbyte + 1);
         zmsgwr_ (msgbuf, &n7);
         sprintf (msgbuf,"ZFI2: BEYOND EOF = %ld", sizebytes);
         }
      else {
         sprintf (msgbuf,"ZFI2: REQUEST FOR BYTES %d THRU = %d",
            firstbyte + 1, lastbyte + 1);
         zmsgwr_ (msgbuf, &n7);
         sprintf (msgbuf,"ZFI2: BEYOND EOF = %d", sizebytes);
         }
      }
      ...
      if (firstbyte != lastiob+1) {
         if (lseek (fd, firstbyte, 0) == -1) {
            ...
            *ierr = 3;
            if (sizeof(long) < sizeof(off_t))
               sprintf (msgbuf,
               "ZFI2: LSEEK ERROR FILE DESCRIPTOR %d TO POSITION %lld",
                  fd, firstbyte);
            else if (sizeof(int) < sizeof(long))
               sprintf (msgbuf,
               "ZFI2: LSEEK ERROR FILE DESCRIPTOR %d TO POSITION %ld",
                  fd, firstbyte);
            else
               sprintf (msgbuf,
               "ZFI2: LSEEK ERROR FILE DESCRIPTOR %d TO POSITION %d",
                  fd, firstbyte);
            }
         }
