...
/*--------------------------------------------------------------------*/
   ...
   *ierr = 0;
   ...
   else if (ioctl(*fcb,MTCAPABILITY,&capa) != 0) {
      *ierr = 3;
      printf("ZTAP2: error getting drive capabilities");
      perror("ZTAP2");
      }
   else {
#if MT_Debug                               /* NOTE - debug            */
      printf("ZTAP2 at NOP op = %s top.mt_op = %d ierr = %d\n",
         op, top.mt_op, *ierr);
      printf ("ZTAP2: mt_dposn %x mt_dsreg %x mt_erreg %x\n",
         stat.mt_dposn, stat.mt_dsreg, stat.mt_erreg);
      printf ("ZTAP2: mt_type %x mtsubtype %x mtcapablity %x\n",
         stat.mt_type, capa.mtsubtype, capa.mtcapablity);
#endif
      if (strcmp(op,"REWI") == 0)
         top.mt_op = MTREW;
         ...
         top.mt_op = MTBSF;
      else if (strcmp(op,"ADVR") == 0)      /* May not detect EOFs    */
         top.mt_op = MTFSR;
      else if (strcmp(op,"BAKR") == 0)      /* May not detect EOFs    */
         top.mt_op = MTBSR;
                                            /* WEOF and BEGW          */
            ...
            if (ioctl(*fcb,MTIOCGET,&stat) != 0) {
               *ierr = 3;
               ...
               printf ("ZTAP2: mt_dposn %x mt_dsreg %x mt_erreg %x\n",
                  stat.mt_dposn, stat.mt_dsreg, stat.mt_erreg);
               printf (
                  "ZTAP2: mt_type %x mtsubtype %x mtcapablity %x\n",
                  stat.mt_type, capa.mtsubtype, capa.mtcapablity);
#endif
               if (stat.mt_dposn & MT_BOT) *ierr = 5;
...
#if MT_Debug                               /* NOTE - debug            */
                  printf (
                     "ZTAP2 at NOP op = %s top.mt_op = %d ierr = %d\n",
                     op, top.mt_op, *ierr);
                  printf (
                     "ZTAP2: mt_dposn %x mt_dsreg %x mt_erreg %x\n",
                     stat.mt_dposn, stat.mt_dsreg, stat.mt_erreg);
                  printf (
                     "ZTAP2: mt_type %x mtsubtype %x mtcapablity %x\n",
                     stat.mt_type, capa.mtsubtype, capa.mtcapablity);
         ...
         if (capa.mtcapablity & MTCAN_SPEOD) {
            top.mt_count = 1;
            top.mt_op = MTEOM;
            ...
            *count = -i - 1;
            if (*ierr == 3) {
               ...
               printf (
                  ...
                  stat.mt_type, capa.mtsubtype, capa.mtcapablity);
#endif
               if (stat.mt_dposn & MT_BOT)
                  *ierr = 5;
               else if (stat.mt_dposn & MT_EOT)
                  *ierr = 6;
               else if (stat.mt_dposn & MT_EOD)
                  *ierr = 6;
               else if (stat.mt_dposn & MT_FMK)
                  *ierr = 4;
               }
            top.mt_op = MTBSF;
            top.mt_count = 2;
