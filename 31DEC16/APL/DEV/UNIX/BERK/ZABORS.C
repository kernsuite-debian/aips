...
/*  BERK version - differs from APLUNIX version in call sequence      */
/*  although the arguments code and scp are not used.  It also differs*/
/*  in assuming a system *sys_siglist set of strings to describe the  */
/*  signals.  Calss signal, not our Posix signel.                     */
/*--------------------------------------------------------------------*/
int isig, code;
...
/*--------------------------------------------------------------------*/
                                        ...
                                        /* a recursive call           */
   if (num_calls != 0) {
      fprintf (stderr,"In ZABORS recursive code - DIE! %d \n",isig);
