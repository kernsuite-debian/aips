...
zcrea2_(pname, ablocks, qcreat, asize, syserr, ierr)
/*--------------------------------------------------------------------*/
/*! create the specified disk file                                    */
/*# IO-basic                                                          */
/*--------------------------------------------------------------------*/
/*;  Copyright (C) 1995, 1997, 2000, 2011                             */
...
/*                        4 => other                                  */
/*  4.2 bsd UNIX version (uses "statfs").  Can handle filesystems     */
/*  larger than 2 Gigabytess, files up to 2 Gigabytes.                */
/*--------------------------------------------------------------------*/
char pname[MAXPNAME];
...
/*--------------------------------------------------------------------*/
{
   ...
   int llen, xlen, xlnb, jerr, fd, tries, lockfd, tdelay,
      iexcl = 1, n6 = 6, n8 = 8;
   off_t bytes;
   int blocks, chunks, rest, xfer, start;
   register int i, j, k;
   float delay = 2.0;
   char lognam[MAXPNAME], xlated[MAXAPATH], filename[MAXPNAME],
      lname[MAXPNAME], lockname[MAXAPATH], pathname[MAXAPATH],
      msgbuf[80];
   char cmd[128];
   char *buff = NULL;
   struct stat statbuf;
         ...
         if (stat (xlated, &statbuf) == -1) {
            ...
            *ierr = 4;
            perror ("ZCREA2 (stat)");
                                        ...
                                        /* target file system.        */
         else if (statfs (xlated, &statfsbuf) == -1) {
            *syserr = errno;
            *ierr = 4;
            perror ("ZCREA2 (statfs)");
                                        ...
                                        /* Convert number of AIPS-    */
                                        /* bytes requested to local   */
                                        /* disk block units.  Avoid   */
                                        /* overflow of 32-bit ints on */
                                        /* large (2+Gig) partitions!  */
                                        /* THIS FAILS FOR >2G FILES!! */
            bytes = *ablocks * Z_nbpab;
            bytes = bytes * 512;
