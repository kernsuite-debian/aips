...
/*--------------------------------------------------------------------*/
{
...
#ifndef HAVE_LINUX_GLIBC
  struct sgttyb setio;                  /* Needed to change terminal */
                                        /* characteristics. */
#else
  struct termios setio;                 /* Needed to change terminal */
                                        /* characteristics. */
#endif
  ...
  int fd;                               /* File descr. of tektool window. */
...
/*--------------------------------------------------------------------*/
  ...
  /* Open up the window to change its characteristics. Then close it. */
...
#ifndef HAVE_LINUX_GLIBC
      /* Change the terminal characteristics to RAW mode and no ECHO. */
   if (ioctl (fd, TIOCGETP, &setio) == -1) {
      perror ("tekserver: Unable to get terminal characteristics");
      sleep (OOPSDELAY);
      exit (errno);
      }

   setio.sg_flags = RAW | setio.sg_flags;
   setio.sg_flags = ECHO ^ setio.sg_flags;

   if (ioctl (fd, TIOCSETP, &setio) == -1) {
      perror ("tekserver: Unable to set terminal characteristics");
      sleep (OOPSDELAY);
      exit (errno);
      }
#else  /* HAVE_LINUX_GLIBC */
   if (tcgetattr (fd, &setio) == -1) {
      perror ("tekserver: Unable to get terminal characteristics");
      sleep (OOPSDELAY);
      exit (errno);
      }
   ...
   if ((device = getenv(getenv("TVDEV"))) == NULL) {
                                        /* undefined                  */
      fprintf (stderr, "TEKserver: NO SUCH DEVICE = TVDEV\n");
      sleep (OOPSDELAY);
      exit(errno);
      }
   ...
   if (strncmp(device,"sssin",5) == 0) {
      ...
      fprintf (stderr, "Enter TEKSERV, Inet domain\n");
      ...
      if ((device = getenv(getenv("TKDEV"))) == NULL) {
                                        /* undefined                  */
         fprintf (stderr, "TEKserver: NO SUCH DEVICE = TKDEV\n");
         sleep (OOPSDELAY);
         exit(errno);
         }
      ...
      fprintf (stderr, "Enter TEKSERV, Unix (local) domain\n");
      ...
      if (! finished) {
         while ((ic = read (snew, buf, BUFLEN)) > 0) {
            ...
            if (! finished) {
               ...
               else if ((buf[ic-1] == '\002') &&
                  (buf[ic-2] == buf[ic-1])) {
                  if (write (snew, catalog, 1024) != 1024) perror
                    ("TEKserver: unable to write catalog to client");
                  ic = ic - 2;
                  }
               else if ((buf[ic-1] == '\003') &&
                  (buf[ic-2] == buf[ic-1])) {
                  ic = 0;
                  finished = 1;
                  fprintf (stderr, "TEKserver told to die\n");
                  }
            ...
            if (! finished) {
               if ((buf[ic-2] == '\033') && (buf[ic-1] == '\032')) {
                  ...
                  if (write (snew, buf, 5) != 5) {
                     perror
                        ("TEKserver: Unable to write back to client");
                     finished = 1;
                     }
                  }
               }
            if (! finished) {
               if (j == 1) {
                  if (write (snew, buf, 2) != 2) perror
                     ("TEKserver: unable to write ACK to client");
                  else {
                     if ((ic = read (snew, buf, BUFLEN)) == 1024) {
                       ...
                       ("TEKserver: unable to write catalog to client");
                        ...
                        fprintf (stderr,
                           "TEKserver: catalog read wrong length %d\n",
                            ic);
                     }
                  }
               }
